...
type v = (Pvac_ffi.cipher, Pvac_ffi.pubkey) value
type instr = (Pvac_ffi.cipher, Pvac_ffi.pubkey) op
...
let create_state ?(limit=1_000_000) ?(ctx=default_ctx) ?(depth=0) ?(is_view=false)
    ...
    ~caller ~origin ~address ~value ~storage () =
  {
    ...
    logs = ref [];
    ctx;
    int_mode = Int_work.select
      ~activate:(Option.map Z.of_int int_work_epoch)
      ~epoch:(Z.of_int ctx.current_epoch);
    undo_stack = [];
    undo_id = 0;
    call_depth = depth;
    return_stack = [];
...
let max_fhe_proof_bytes = Contract_vm_host.max_fhe_proof_bytes
let max_fhe_cipher_bytes = 1_048_576
let max_fhe_pubkey_bytes = 18_000_000
let fhe_decode_bytes_per_effort = 16

let fhe_verifier_cipher_allowed = Contract_vm_host.cipher_ok
...
let op_effort st op =
  let binary kind left right =
    Int_work.cost st.int_mode kind
      (to_z (getr st left)) (to_z (getr st right))
  in
  let unary kind source =
    Int_work.cost st.int_mode kind (to_z (getr st source)) Z.zero
  in
  match op with
...
let object_apply_dyn_cost writes =
  List.fold_left
    (fun acc write ->
      ...
      | Contract_vm_host.Set (_key, value) ->
        10 + (String.length value / 32)
      | Contract_vm_host.Del _ ->
        5
      in
...
let apply_object_write st = function
  | Contract_vm_host.Set (key, value) ->
...
let exec_one st op =
  ...
  | STATE_PATH_KEY (rd, rs) ->
    begin
      match Contract_vm_host.state_path_key (to_string (getr st rs)) with
      | Some path_key ->
      ...
      begin
        match
          Contract_vm_host.apply_object
            ...
            ~proof_kind:(to_string (getr st rproof_kind))
            ~proof_receipt_hash:(to_string (getr st rproof_receipt_hash))
    ...
    if not (view_guard st) then false
      ...
      if String.length bytecode_raw < 12 then revert st
        ...
        (match st.ctx.deploy_contract st.address bytecode_raw nonce (st.call_depth + 1) [] with
         ...
         | Error e ->
           Contract_vm_host.spawn_error
             ~reason:e
             ~octets:(String.length bytecode_raw);
    ...
    if not (st.ctx.allow_fhe_capability Fhe_verify_zero_cap) then
      ...
      revert st
       ...
       | Some pk, Some ct, Some proof_bytes
         when Contract_vm_host.cipher_ok ct ->
         ...
         if raw_len > max_fhe_proof_bytes then
           ...
           let ok =
             with_fhe_verifier_lane (fun () ->
               match Contract_vm_host.zero_proof proof_bytes with
               | None -> false
               | Some proof ->
                 Contract_vm_host.verify_zero
                   ~pubkey:pk
                   ~cipher:ct
                   ~proof)
           in
    ...
    (match to_pubkey (getr st rpk), to_cipher (getr st rct), to_bytes (getr st rproof) with
     | Some pk, Some ct, Some proof_bytes
       ...
       if raw_len > max_fhe_proof_bytes then
         ...
         let ok =
           with_fhe_verifier_lane (fun () ->
             match Contract_vm_host.range_proof proof_bytes with
             | None -> false
             | Some proof ->
               Contract_vm_host.verify_range
                 ~pubkey:pk
                 ~cipher:ct
                 ~proof)
         in
     ...
     | Some vk_in, Some proof_in, Some inputs_in ->
       ...
       if String.length vk_raw > max_zk_vk_bytes
         ...
         (match groth16_public_inputs vk_raw with
          ...
          | Some _ ->
            let ok =
              Contract_vm_host.groth16
                ~key:(Bytes.of_string vk_raw)
                ~proof:(Bytes.of_string proof_raw)
                ~inputs:(Bytes.of_string inputs_raw)
            in
            setr st rd (VBool ok);
            true)
     | _ -> revert st)
       ...
       | Some pk, Some ct, Some proof_bytes, Some commit_bytes
         ...
         if raw_len > max_fhe_proof_bytes then
           ...
           let ok =
             with_fhe_verifier_lane (fun () ->
               match
                 Contract_vm_host.zero_proof proof_bytes,
                 Contract_vm_host.commitment commit_bytes
               with
               | Some proof, Some commitment ->
                 Contract_vm_host.verify_claim
                   ~pubkey:pk
                   ~cipher:ct
                   ~proof
                   ~commitment
...
let rec run state program =
  ...
  | Running -> run state program
  | Finished -> true
  | Refused -> false

module Verifier = Vm_isa.Verifier